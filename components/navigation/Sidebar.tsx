  ...
  return (
    <div className="space-y-10">
      <div className="border flex flex-col items-center justify-center p-5 space-y-5">
        <Link href="/about">
          <Image src="/joytabi.png" width={120} height={120} alt="avatar" className="rounded-full" priority={false} />
        </Link>

        <div className="font-bold text-xl">じょい旅</div>

        <div className="text-sm">2025年1月から世界一周をしているアラサー夫婦。役に立つ情報を発信していきます。</div>
        ...
        <div className="flex items-center justify-center gap-4">
          ...
          <Link href="https://www.youtube.com/@joitabi" className="hover:opacity-80">
            <FaYoutube size={30} />
          </Link>
        </div>
      </div>
        ...
        <div className="border text-sm">
          {isLoading ? (
            ...
            latestBlogs.map((blog, index, array) => (
              <Link
                href={`/blog/${blog.id}`}
                className={`grid grid-cols-3 hover:text-gray-500 group ${index !== array.length - 1 ? "border-b" : ""}`}
                key={index}
              >
          ...
          {isLoading ? (
            ...
            categoryCounts.map((category, index, array) => (
              <Link
                ...
                className={`p-3 flex items-center justify-between hover:text-gray-500 ${index !== array.length - 1 ? "border-b" : ""}`}
                key={index}
              >
